...
def main() -> None:
    ...
    if not sys.stdin.isatty():
        ...
        return
    console.print(
        "[dim]Interactive mode: after each [/][bold]>[/][dim] prompt, type a question and press Enter. "
        "The model prints an answer. To stop, press Enter once with no text (empty line).[/]"
    )
    while True:
        try:
            line = input("> ").strip()
